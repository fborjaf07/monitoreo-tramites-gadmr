...
async def login(page):
    """Login CAS - navega directo al callback del e-DOC"""
    ...
    print("Paso 2: Llenando credenciales...")
    ...
    # Si seguimos en el CAS, navegar manualmente al e-DOC con el service
    if "egobedoc" not in page.url:
        print("Paso 4: Navegando manualmente al e-DOC...")
        await page.goto(BASE_URL + "/", timeout=60000, wait_until="networkidle")
        await page.wait_for_timeout(3000)
        print(f"URL tras goto /: {page.url}")

    # Si aún estamos en login, intentar con goto directo al callback
    if "cas/login" in page.url or "egobedoc" not in page.url:
        print("Paso 5: Intentando goto directo al callback...")
        await page.goto(SERVICE, timeout=60000, wait_until="networkidle")
        await page.wait_for_timeout(3000)
        print(f"URL tras callback: {page.url}")

    texto = await page.inner_text("body")
    print(f"Body final[0:300]: {texto[:300].replace(chr(10),' ')}")

    if "cas/login" in page.url or "Enter Username" in texto:
