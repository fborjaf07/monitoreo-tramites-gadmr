...
# Secciones a monitorear y sus URLs
SECCIONES = {
    "Bandeja de entrada": f"{BASE_URL}/issues?assigned_to_id=me&status_id=o&sort=updated_on%3Adesc",
    "Enviados":           f"{BASE_URL}/issues?author_id=me&sort=updated_on%3Adesc",
    "Reasignados":        f"{BASE_URL}/issues?assigned_to_id=me&status_id=*&sort=updated_on%3Adesc&f[]=assigned_to_id&op[assigned_to_id]=%3D&f[]=author_id&op[author_id]=%21",
}
...
async def extraer_lista_seccion(page, seccion, url_base):
    """
    Navega la paginación de una sección y devuelve
    lista de dicts con {numero, descripcion, seccion}.
    """
    numeros = []
    pagina  = 1
    ...
    while True:
        ...
        print(f"  [{seccion}] Página {pagina}...")
        await page.goto(url, wait_until="networkidle", timeout=60000)
        await page.wait_for_timeout(2000)

        texto = await page.inner_text("body")

        # Buscar números de trámites en la lista
        # Patron: links tipo /issues/NNNN o texto #NNNN
        patron_num = re.findall(r'/issues/(\d+)', await page.content())
        nuevos = list(dict.fromkeys(patron_num))  # dedup manteniendo orden

        if not nuevos:
            break

        # También intentar extraer descripción desde el texto de la página
        for num in nuevos:
            if num not in [n["numero"] for n in numeros]:
                numeros.append({
                    "numero":      num,
                    "seccion":     seccion,
                    "descripcion": ""  # se llenará al entrar al detalle
                })

        # ¿Hay página siguiente?
        siguiente = await page.query_selector('a[rel="next"], .next_page a, a:has-text("Siguiente"), a:has-text("Next")')
        if not siguiente:
            # Verificar por texto en el contenido
            if "Siguiente" not in texto and "next" not in texto.lower():
                break
            else:
                # Intentar clic si existe el elemento
                try:
                    await page.click('a:has-text("Siguiente")', timeout=3000)
                    await page.wait_for_timeout(2000)
                    pagina += 1
                    continue
                except Exception:
    ...
    print(f"  [{seccion}] {len(numeros)} trámites encontrados")
    return numeros
...
# ============================================================
async def extraer_detalle(page, numero, seccion, descripcion_entrada=""):
    resultado = {
        "numero":            numero,
        "seccion":           seccion,
    ...
    try:
        ...
        # Calcular tiempo desde último movimiento
...
async def ejecutar():
    ...
    async with async_playwright() as p:
        ...
        for seccion, url in SECCIONES.items():
            ...
            for item in items:
                if item["numero"] not in vistos:
                    ...
                    # Si ya existe de otra sección, agregar sección secundaria
                    pass
        ...
        for i, item in enumerate(todos_lista, 1):
            ...
            if detalle["error"]:
                ...
                if movs:
                    ultimo = movs[-1]
                    print(f"  ✓ {len(movs)} movimientos | Último: {ultimo['nombre']} → {ultimo.get('enviado_a','')}")
    ...
    print(f"\n✓ {ARCHIVO_JSON} guardado ({len(tramites_detalle)} trámites)")
    ...
    alertas_payload = {
        ...
        "alertas":       alertas,
        "mensaje_resumen": (
            f"ALERTA TRÁMITES — {datetime.now().strftime('%d/%m/%Y %H:%M')}\n"
            f"Se encontraron {len(alertas)} trámite(s) sin movimiento por más de {HORAS_ALERTA}h."
    ...
    print(f"✓ {ARCHIVO_ALERTAS} guardado ({len(alertas)} alertas)")
